...
const PRODUCTS_URL = "https://odyssey-app-server.vercel.app/products";
const PAGE_SIZE = 9;
...
function RatingStars({ rating = 0, count }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5">
      ...
      {count != null && (
        <span className="text-xs text-gray-400 dark:text-gray-500">({count} reviews)</span>
      )}
    </div>
  );
}
...
function Pagination({ page, totalPages, onPageChange, disabled }) {
  ...
  const active = "border border-indigo-600 bg-indigo-600 text-white dark:border-indigo-500 dark:bg-indigo-500";
  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-center justify-center gap-1">
        <button type="button" className={`${btn} ${idle}`} disabled={disabled || page <= 1} onClick={() => go(page - 1)}>
          Previous
        </button>
        {start > 1 && (
          <>
            <button type="button" className={`${btn} ${page === 1 ? active : idle}`} disabled={disabled} onClick={() => go(1)}>
              1
            </button>
...
export default function Items() {
  ...
  const filtered = useMemo(() => {
    ...
    if (price === "low") list = [...list].sort((a, b) => (a.price ?? 0) - (b.price ?? 0));
    else if (price === "high") list = [...list].sort((a, b) => (b.price ?? 0) - (a.price ?? 0));
    return list;
      ...
      <main className="min-h-screen bg-gray-50 dark:bg-gray-950">
        <div className="border-b border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900">
          <div className="mx-auto flex max-w-6xl flex-wrap items-center gap-2 px-6 py-3 text-xs text-gray-500 dark:text-gray-400">
            <Link href="/" className="transition hover:text-gray-800 dark:hover:text-gray-200">
              Home
            </Link>
        ...
        <div className="mx-auto max-w-6xl px-6 py-10">
              ...
              <p className="text-right text-sm text-gray-500 dark:text-gray-400">
                {rangeLabel ? (
                  <>
                    ...
                    <span> of </span>
                    <span className="font-semibold text-gray-800 dark:text-gray-200">{rangeLabel.total}</span>
                ...
                {hasActiveFilters && items.length !== filtered.length && (
                  <span className="text-gray-400 dark:text-gray-500"> · {items.length} total</span>
                )}
              </p>
            )}
          </header>
          ...
          <div ref={catalogTopRef} className="scroll-mt-24">
            <div className="mt-8 grid gap-6 sm:grid-cols-2 lg:grid-cols-3" aria-busy={loading}>
            {loading &&
                ...
                >
                  ...
                  <div className="flex flex-1 flex-col gap-3 p-4">
                    ...
                    <div className="flex flex-wrap items-end gap-2">
                      <span className="text-lg font-bold text-gray-900 dark:text-gray-50">৳{item.price}</span>
                      {item.oldPrice != null && item.oldPrice > item.price && (
                        <span className="text-sm text-gray-400 line-through dark:text-gray-500">৳{item.oldPrice}</span>
                      )}
                    </div>
            ...
            {!loading && !error && filtered.length === 0 && (
              <div className="col-span-full flex flex-col items-center justify-center rounded-2xl border border-dashed border-gray-300 bg-gray-50/50 px-6 py-16 text-center dark:border-gray-700 dark:bg-gray-900/50">
                {items.length === 0 ? (
                  <>
                    <p className="text-sm font-medium text-gray-700 dark:text-gray-300">No products to show</p>
                    <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">Check back later or add items from the dashboard.</p>
                  </>
                ) : (
                  <>
                    <p className="text-sm font-medium text-gray-700 dark:text-gray-300">No items match your filters</p>
            ...
            {!loading && !error && totalPages > 0 && (
              <Pagination page={currentPage} totalPages={totalPages} onPageChange={setPage} disabled={loading} />
            )}
          </div>
        </div>
      </main>
